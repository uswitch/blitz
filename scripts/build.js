...
function build(previousFileSizes) {
  // Check if blitz.config.js exists
  let blitz = {};
  try {
    blitz = require(paths.appBlitzConfig);
  } catch (e) {
    console.log('Failed to load blitz.config.js.', e);
  }

  if (blitz.clearConsole === false || !!blitz.host || !!blitz.port) {
    logger.warn(`Specifying options \`port\`, \`host\`, and \`clearConsole\` in blitz.config.js has been deprecated.
Please use a .env file instead.

${blitz.host !== 'localhost' && `HOST=${blitz.host}`}
${blitz.port !== '3000' && `PORT=${blitz.port}`}
`);
  }

  // Create our production webpack configurations and pass in blitz options.
  let clientConfig = createConfig('web', 'prod', blitz);
  let serverConfig = createConfig('node', 'prod', blitz);
